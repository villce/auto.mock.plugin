...
import com.esatmoney.mock.data.json.JsonConfigList;
 ...
 * @Version: 1.0
 */
public class BuildClassMethodParameterImpl {
    /**
     * 获取的参数
     * 参数组装-方法参数
     *
    ...
    public static void build(JavaMethod javaMethod, JavaClassInfo javaGenInfoModel, JavaMethodDTO javaMethodDTO, JavaClass javaClass) {
        ...
        for (JavaParameter javaParameter : parameterList) {
            ...
            LoggerUtil.info(BuildClassMethodParameterImpl.class, "当前的参数信息: " + javaParameterDTO);
            ...
            if (javaParameterDTO.getValue() == null) {
                ...
                LoggerUtil.info(BuildClassMethodParameterImpl.class, "自定义类型：" + typeToStr + "，cls：" + (parameterJavaClass == null ? "null" : parameterJavaClass.getFullyQualifiedName()));
                ...
                if (parameterJavaClass != null) {
                    ...
                    if (InitConstant.FULLY_COLLECTION_VALUE_IMPORT.keySet().contains(typeToStr)) {
                        ...
                        javaParameterDTO.setInter(true);
                        LoggerUtil.debug(BuildClassMethodParameterImpl.class, "参数类型为FULLY_COLLECTION_VALUE_IMPORT中类型，javaParameterDTO:" + javaParameterDTO);
                    } else if (InitConstant.SPECIAL_VALUE.keySet().contains(typeToStr)) {
                        // 设置为指定赋值
                        javaParameterDTO.setSpecial(true);
                        javaParameterDTO.setSubClassFullyType(InitConstant.SPECIAL_VALUE.get(typeToStr));
                        javaParameterDTO.setSubClassType(InitConstant.getAbbreviation(InitConstant.SPECIAL_VALUE.get(typeToStr)));
                        ...
                        LoggerUtil.debug(BuildClassMethodParameterImpl.class, "javaClassList1:" + javaClassList + ",自定义类型" + typeToStr);
                        ...
                        LoggerUtil.debug(BuildClassMethodParameterImpl.class, "获取的枚举值：" + javaFieldList + "，javaClass=" + parameterJavaClass);
                        ...
                        LoggerUtil.debug(BuildClassMethodParameterImpl.class, "superJavaClass:" + superJavaClass.getFullyQualifiedName() + ",自定义类型" + typeToStr + "，自定义类型中的类型：" + javaParameterDTOList);
                        javaParameterDTO.setJavaParameterDTOList(javaParameterDTOList);
                    }
            ...
            LoggerUtil.debug(BuildClassMethodParameterImpl.class, "生成完参数值的设定:" + javaParameterDTO);
    ...
    private static void addParameterToList(List<JavaParameterDTO> javaParameterDTOList, JavaClass javaClass, JavaClassInfo javaGenInfoModel) {
        ...
        if (javaFields.isEmpty()) {
            LoggerUtil.warn(BuildClassMethodParameterImpl.class, "获取的类下的属性为空，可能是由于不在同一个项目，类：" + javaClass);
        }
            ...
            if (value == null) {
                ...
                if (javaClass1 != null) {
                    ...
                    if (InitConstant.COLLECTION_VALUE_IMPORT.keySet().contains(fieldType)) {
                        ...
                        if (!javaClassList.isEmpty()) {
                            //处理List
                            LoggerUtil.info(BuildClassMethodParameterImpl.class, "获取的属性类型为:" + fieldType);

                            //取第一个
                            JavaClass javaClass2 = javaClassList.get(0);
                            LoggerUtil.info(BuildClassMethodParameterImpl.class, "获取的属性父类信息：" + javaClass2.getFullyQualifiedName() + "，fullyTypeName=" + fullyTypeName + "，javaClass2.getName=" + javaClass2.getName() + "," + javaClass2.getSimpleName());
                            javaParameterDTO1.setCustomType(true);
                            fullyTypeName = javaClass2.getFullyQualifiedName();
                            javaParameterDTO1.setFullyType(fullyTypeName);
                            if ("List".equals(fullyTypeName)) {
                                LoggerUtil.info(BuildClassMethodParameterImpl.class, "javaParameterDTOList=" + javaParameterDTOList + ",,,javaParameterDTO1=" + javaParameterDTO1);
                            }
    ...
    private static void javaParameterDTOSetJsonValue(List<JavaParameterDTO> javaParameterDTOS) {
        ...
        for (JavaParameterDTO parameterDTO : javaParameterDTOS) {
            LoggerUtil.info(BuildClassMethodParameterImpl.class, "开始进行json赋值：" + JSON.toJSONString(parameterDTO));
            ...
            LoggerUtil.info(BuildClassMethodParameterImpl.class, "开始进行json赋值,获取到配置：" + JSON.toJSONString(jsonConfig));
            if (jsonConfig == null) {
                return;
            }
            boolean isOpen = jsonConfig.getOpen();
            if (!isOpen) {
                return;
            }
    ...
    private static void methodValue(JavaParameterDTO parameterDTO, List<JavaParameterDTO> javaParameters, List<JsonConfigList> jsonConfigLists) {
        for (JsonConfigList jsonConfigList : jsonConfigLists) {
            ...
            if (JsonConfigListScopeEnum.METHOD.getValue().equals(scope)) {
                ...
                if (StringUtil.isEmpty(scopeValue)) {
                    LoggerUtil.warn(BuildClassMethodParameterImpl.class, "【mock-test】json配置的作用域为METHOD，未配置scopeValue,jsonConfigList=" + JSON.toJSONString(jsonConfigList));
                    continue;
                }
    ...
    private static void classValue(JavaParameterDTO parameterDTO, List<JavaParameterDTO> javaParameters, List<JsonConfigList> jsonConfigLists) {
        for (JsonConfigList jsonConfigList : jsonConfigLists) {
            ...
            if (JsonConfigListScopeEnum.CLASS.getValue().equals(scope)) {
                ...
                String scopeValue = jsonConfigList.getScopeValue();
                if (StringUtil.isEmpty(scopeValue)) {
                    LoggerUtil.warn(BuildClassMethodParameterImpl.class, "【primo-generator-mock-test】json配置的作用域为CLASS，未配置scopeValue,jsonConfigList=" + JSON.toJSONString(jsonConfigList));
                    continue;
                }
    ...
    private static void packageValue(JavaParameterDTO parameterDTO, List<JavaParameterDTO> javaParameters, List<JsonConfigList> jsonConfigLists) {
        for (JsonConfigList jsonConfigList : jsonConfigLists) {
            ...
            if (JsonConfigListScopeEnum.PACKAGE.getValue().equals(scope)) {
                ...
                String scopeValue = jsonConfigList.getScopeValue();
                if (StringUtil.isEmpty(scopeValue)) {
                    LoggerUtil.warn(BuildClassMethodParameterImpl.class, "【primo-generator-mock-test】json配置的作用域为PACKAGE，未配置scopeValue,jsonConfigList=" + JSON.toJSONString(jsonConfigList));
                    continue;
                }
    ...
    private static void jsonValueSet(List<JavaParameterDTO> javaParameters, JsonConfigList jsonConfigList) {
        ...
        LoggerUtil.info(BuildClassMethodParameterImpl.class, "开始进行json赋值,jsonObject：" + jsonObject);
        ...
        for (JavaParameterDTO javaParameter : javaParameters) {
            ...
            LoggerUtil.info(BuildClassMethodParameterImpl.class, "设置的值为：" + javaParameter);
        }
    }
